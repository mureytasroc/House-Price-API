...
import random
import string
from housepriceapi.datadesc import *

test_api_dict = {'cont_features': [[feature_names_dict[c], c] for c in cont_features],
                 'cat_features': [[feature_names_dict[c], c, cat_options_dict[c]]
                                  for c in cat_features],
                 'features_desc': [(feature_names_dict[f],
                                    feature_desc_dict[f])
                                   for f in features]}

model = catboost.CatBoost()
model.load_model('housepriceapi/CatboostModel.cbm', format='cbm')
...
def test_api_view(request):
    ...
    if request.method == "POST":
        if not request.POST['api_key'] or \
                not Profile.objects.filter(api_key=request.POST['api_key']):
            return HttpResponse('API Key is Invalid', status=401)
        vf = validate_features(request)
        if vf is not None:
            return vf
        return render(request, 'testapi.html', {**{'returned_data': get_home_price_json(request)},
                                                **test_api_dict})
